...
# coord
coord_df <- read.csv('all_coord.csv',
                     stringsAsFactors = F) %>% 
...
chelsa_df <- expand.grid( variable = c('tmean','tmax','tmin','prec'),
                          year     = c(1979:2013),
...
file_dest  <- paste0( pp_v, gsub('^tmean|^tmax|^tmin|^prec',
                                 '',file_names ) )
...
# put it all out
write.csv(clim_df,
          paste0(out_file, '_7913_',job_n,'.csv'),
          row.names=F)
